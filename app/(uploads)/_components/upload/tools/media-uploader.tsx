...
import { ImageBlock, VideoBlock } from '@/types/shot'
...
const MediaUploader = ({ rootBlock, block, index }: Props) => {
            ...
            if (checkedFile) {
                ...
                if (res) {
                    if (rootBlock) {
                        ...
                        dispatch(setRootBlock({ type: type, link: res }))
                        setLoading(false)
                    } else {
                        const block = { type: type, link: res } as VideoBlock | ImageBlock
    ...
    const uploadFile = async(file: File) => {
        if (user && draftId !== '') {
            ...
            const res = await fileAPI.upload.file(link, file)
            return res
        } return null
    }
    const removeThumbnail = async() => {
        ...
        if (rootBlock) {
            if (draft.rootBlock.type === 'image' || draft.rootBlock.type === 'video') {
                ...
                const res = await fileAPI.upload.delete(draft.rootBlock.link)
                if (res) {
            ...
            if (blockIndex > -1) {
                ...
                const res = await fileAPI.upload.delete(block.link)
                if (res) {
    ...
    const stableLink = (!rootBlock ? block && block.link ? block.link : (draft.rootBlock.type === 'video' || draft.rootBlock.type === 'image') ? draft.rootBlock.link : '' : '')
    return (
