...
const ModalUploaderSlice = createSlice({
  ...
  reducers: {
    setFinalTouchModal(
      state,
      { payload, type }: { payload: InitState["finalTouchModal"]; type: string }
    ) {
      state.finalTouchModal = payload;
    },
    setStickerPicker(
      state,
      { payload, type }: { payload: InitState["stickerPicker"]; type: string }
    ) {
      state.stickerPicker = payload;
    },
  },
});
export const { setFinalTouchModal, setStickerPicker } =
  ModalUploaderSlice.actions;
