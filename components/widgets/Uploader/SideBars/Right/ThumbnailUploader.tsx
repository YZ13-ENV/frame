...
import { cdn, filer } from '@darkmaterial/api/helpers'
...
const ThumbnailUploader = () => {
    ...
    const onFile = async(file: File) => {
        if (user && draftId) {
            const checkedFile = filer.check(user.uid, draftId, file)
            if (localThumbnail) {
            ...
            if (checkedFile) {
                ...
                const res = await fileAPI.upload.thumbnail(user.uid, draftId, file)
                if (res) {
                    setLoading(false)
    ...
    const removeThumbnail = async() => {
        ...
        if (draft.thumbnail && draft.thumbnail.link !== '') {
            ...
            const res = await fileAPI.delete(draft.thumbnail.link)
            if (res) {
        ...
        <div className="relative w-full aspect-[4/3] rounded-xl bg-black border overflow-hidden border-transparent">
            ...
            }
            {
                ...
                ? <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    {
                        (localMediaType ? localMediaType === 'image' : !draft.thumbnail.link.endsWith('.mp4'))
                        ? <Image src={localThumbnail || cdn(draft.thumbnail.link)} 
                        ...
                        : <video className={`transition-all w-full h-full ${loading ? 'brightness-50' :'brightness-100'}`}>
                            <source src={localThumbnail || cdn(draft.thumbnail.link)} />
                        </video>
                    }
                </motion.div>
                : disabled
                ...
                <div className="relative flex flex-col items-center justify-center w-full h-full gap-4 p-4">
                    ...
                    { 
                        (thumbnail.savedFile && thumbnail.forcedType === 'image') && <>
                            ...
                            <Button onClick={() => uploadSavedFile(thumbnail.savedFile)} className='z-20' size='sm'>Загрузить из главного блока</Button> 
                        </>
                    }
                </div>
            }
