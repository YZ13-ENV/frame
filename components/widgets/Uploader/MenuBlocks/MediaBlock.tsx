...
import { BiImage, BiVideo, BiX } from 'react-icons/bi'
import type { MediaBlock } from '@darkmaterial/core/types'
...
const MediaBlock = ({ block, index }: Props) => {
    ...
    const deleteBlock = async() => {
        const res = await fetch(`${host}/files/file?link=${block.link}`, { method: "DELETE" })
        if (res.ok) {
    ...
    return (
        <div className="flex items-center justify-between w-full gap-2 p-2 pl-3 border h-14 rounded-xl border-neutral-800 hover:bg-neutral-900">
            <div className="flex items-center gap-2 w-fit h-fit group">
                ...
                <span className='text-sm text-neutral-300'>{block.type === 'image' ? 'Картинка' : 'Видео'}</span>
            </div>
        </div>
    )
}
