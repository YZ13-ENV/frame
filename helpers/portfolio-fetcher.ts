import type { DocTeam } from "@darkmaterial/api";
import { ShortUserData, team, user } from "@darkmaterial/api";

type UserVariant = { type: "user" } & ShortUserData;
type TeamVariant = { type: "team" } & DocTeam;
export const fetch_author = async (
  id: string
): Promise<UserVariant | TeamVariant | null> => {
  const byId = await user.byId.short(id);
  const byNickname = (await user.byNick.short(
    id,
    false
  )) as unknown as ShortUserData | null;
...
export const author_config = (author: UserVariant | TeamVariant) => {
  ...
  return {
    ...
    data:
      author.type === "team"
        ? (author as TeamVariant)
        : (author as UserVariant),
  };
};
